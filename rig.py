...
# Initialize variables for calculating reprojection error
total_errorL, total_errorR = 0, 0
total_pointsL, total_pointsR = 0, 0
...
# Replace fisheye calibration with standard pinhole model
logging.info("Performing calibration.")
retL, mtxL, distL, rvecsL, tvecsL = cv2.calibrateCamera(objpointsL, imgpointsL, imgL.shape[::-1], None, None)
retR, mtxR, distR, rvecsR, tvecsR = cv2.calibrateCamera(objpointsR, imgpointsR, imgR.shape[::-1], None, None)
...
for i in range(1, 30):
    ...
    undistorted_imgL = cv2.undistort(imgL, mtxL, distL)
    undistorted_imgR = cv2.undistort(imgR, mtxR, distR)
...
for i, objp in enumerate(objpointsL):
    ...
    # For left camera
    reprojected_imgpointsL, _ = cv2.projectPoints(objp, rvecsL[i], tvecsL[i], mtxL, distL)
    reprojected_imgpointsL = np.squeeze(reprojected_imgpointsL).astype(np.float32)
    ...
    # For right camera
    reprojected_imgpointsR, _ = cv2.projectPoints(objp, rvecsR[i], tvecsR[i], mtxR, distR)
    reprojected_imgpointsR = np.squeeze(reprojected_imgpointsR).astype(np.float32)  # Squeeze dimensions and change type to float32

    # Calculate reprojection error for left camera
    errorL = cv2.norm(np.squeeze(imgpointsL[i]).astype(np.float32), reprojected_imgpointsL, cv2.NORM_L2) / len(reprojected_imgpointsL)
    total_errorL += errorL
    total_pointsL += len(objpointsL[i])

    #print(imgpointsR[i].shape, imgpointsR[i].dtype)
    #print(reprojected_imgpointsR.shape, reprojected_imgpointsR.dtype)

    imgpointsR[i] = np.squeeze(imgpointsR[i]).astype(np.float32)

    # Calculate reprojection error for right camera
    errorR = cv2.norm(imgpointsR[i], reprojected_imgpointsR, cv2.NORM_L2) / len(reprojected_imgpointsR)
    total_errorR += errorR
    total_pointsR += len(objpointsR[i])
...
if total_pointsL > 0:
    mean_errorL = total_errorL / total_pointsL
    logging.info(f"Mean reprojection error for left camera is: {mean_errorL} pixels")

if total_pointsR > 0:
    mean_errorR = total_errorR / total_pointsR
    logging.info(f"Mean reprojection error for right camera is: {mean_errorR} pixels")
